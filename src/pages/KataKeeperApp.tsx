...
export default function KataKeeperApp() {
  ...
  const [sortKey, setSortKey] = useState<
    'updatedAt' | 'createdAt' | 'lastWorkedAt' | 'title' | 'difficulty'
  >('updatedAt');
              ...
              {filtered.map((k) => (
                <tr key={k.id} className="border-t border-slate-100 dark:border-slate-800">
                  ...
                  <td className="px-4 py-3 align-top">
                    <Select
                      value={k.status}
                      onChange={(e) => quickStatus(k, e.target.value as Status)}
                    >
                      {STATUSES.map((s) => (
                        <option key={s} value={s}>
                          {s}
                  ...
                  <td className="px-4 py-3 hidden md:table-cell align-top">
                    ...
                    <div className="text-xs text-slate-500" title={k.lastWorkedAt}>
                      worked {formatRelative(k.lastWorkedAt)}
                    </div>
                  </td>
                  <td className="px-4 py-3 align-top">
                    <div className="flex flex-wrap gap-2">
                      <IconButton onClick={() => stampWorked(k)} title="Mark worked">
                        Worked
                      </IconButton>
          ...
          <div className="relative w-full max-w-2xl rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 shadow-2xl">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex flex-wrap gap-2">
                ...
                {details.difficulty && (
                  <Pill className="border-slate-300 dark:border-slate-600">
                    {details.difficulty}
                  </Pill>
                )}
              </div>
              ...
              <div className="text-xs text-slate-500">
                <div>
                  Created:{' '}
                  <time title={details.createdAt}>{formatRelative(details.createdAt)}</time>
                </div>
                <div>
                  Updated:{' '}
                  <time title={details.updatedAt}>{formatRelative(details.updatedAt)}</time>
                </div>
                <div>
                  Last worked:{' '}
                  <time title={details.lastWorkedAt}>{formatRelative(details.lastWorkedAt)}</time>
                </div>
              </div>
            </div>
          </div>
